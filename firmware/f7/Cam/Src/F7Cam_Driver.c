...
uint8_t CAM_Init(uint8_t format, uint16_t x_res, uint16_t y_res, uint8_t FPS, uint16_t FB_size, uint16_t FIFO_width, uint8_t packet_count, uint8_t jpeg_comp_ratio)
{
  ...
  if(ov5640_ReadID(CAMERA_I2C_ADDRESS_OV5640) == OV5640_ID)
  {
    ...
    //HAL_DCMI_Start_DMA2(&hDcmiHandler, DCMI_MODE_CONTINUOUS, (uint32_t)cam_fb, (CAM_FB_SIZE / 4));//GetSize(CameraCurrentResolution)); //0x1900 /4
...
void BSP_CAMERA_ContinuousStart()
{
  ...
  // while (vsyncCounter < 20) {
  //   currentValue = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_9);
  //   read = false;
  //   if ((currentValue != lastValue) && read == false) {
  //       vsyncCounter++;
  //       printf("VSYNC\n");
  //       read = true;
...
void HAL_DCMI_VsyncEventCallback(DCMI_HandleTypeDef *hdcmi)
{
  ...
  if (loadingCounter > 10) {
    BSP_CAMERA_Stop();
    for(uint32_t i = 0; i < ((total_buffer_fragments/ 4) * FIFO_SIZE) ; i = i + 2) {
  ...
  if (current_frame_end_pos < current_frame_start_pos) {
    ...
    uint16_t bufferEndSize = ((total_buffer_fragments) - current_frame_start_pos) * FIFO_SIZE;
    //printf("pos1:%i\n", current_frame_start_pos);
    //printf("bufferEndSize: %i\n", bufferEndSize / FIFO_SIZE);
    ...
    Event data_ready_part1_event = {
      ...
      //if (loadingCounter >= startOffset) {
        push_event_queue(data_ready_part1_event);
      //}


    //pay load 2: start of buffer to current frame
    //printf("pos2:%i\n", 0);
  ...
  JPEG_Size = bufferEndSize + bufferStartSize;
    ...
    // printf("wholeFrameSize: %i\n\n", wholeFrameSize / FIFO_SIZE);
    // printf("frame first byte: %x/%c\n", cam_fb[(current_frame_start_pos) * FIFO_SIZE], cam_fb[(current_frame_start_pos) * FIFO_SIZE]);
    // printf("prev packet first byte: %x/%c\n", cam_fb[(current_frame_start_pos - 1) * FIFO_SIZE], cam_fb[(current_frame_start_pos - 1) * FIFO_SIZE]);
    // printf("next packet first byte: %x/%c\n", cam_fb[(current_frame_start_pos + 1) * FIFO_SIZE], cam_fb[(current_frame_start_pos + 1) * FIFO_SIZE]);
    // printf("NDTR * 4: %d\n", hdcmi->DMA_Handle->Instance->NDTR * 4);
...
void JPEG_search_Full_Frame(void) {
	        ...
	        for(i=0;i<CAM_FB_SIZE; i++)//search for 0XFF 0XD8 and 0XFF 0XD9, get size of JPG
	        {
	                if((cam_fb[i]==0XFF)&&(cam_fb[i+1]==0XD8))
	                {
	                		...
	                		JPEG_HEX_SIZE_BUFFER[jpeg_frame_counter + 1] = jpglen + padding_counter + 1;
	                		padding_counter = 0;
	                		jpeg_frame_counter++;
