...
void ov5640_Init_JPEG(uint16_t x_res, uint16_t y_res, uint16_t FifoWidth,
  ...
  // CAMERA_IO_Write_OV5640(OV5640_I2C_ADDRESS, 0x3035,0X41); // slow down
  // OV5640 clocks CAMERA_IO_Write_OV5640(OV5640_I2C_ADDRESS, 0x3036,0x79);
  ...
  // calc_sysclk(uint32_t xclk, bool pll_bypass, uint16_t pll_multiplier,
  // uint16_t pll_sys_div, uint16_t pre_div, bool root_2x, uint16_t
  // pclk_root_div, bool pclk_manual, uint16_t pclk_div) calc_sysclk(24000000,
  // false, 0x30, 4, 30, bool root_2x, uint16_t pclk_root_div, bool pclk_manual,
  // uint16_t pclk_div)
}
